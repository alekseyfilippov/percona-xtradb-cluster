...
if [ -z "$PMM_SERVER_NAME" ]; then
	echo >&2 'Error:  You need to specify PMM_SERVER_NAME'
	exit 1
fi

if [ -z "$MYSQL_NODE_NAME" ]; then
	echo >&2 'Error:  You need to specify MYSQL_NODE_NAME'
	exit 1
fi

sudo pmm-agent setup --config-file=pmm-agent.yaml --server-insecure-tls --server-address="$PMM_SERVER_NAME":443 --server-username=admin --server-password=admin
